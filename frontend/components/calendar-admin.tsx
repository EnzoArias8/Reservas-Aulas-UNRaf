...
const safeFormatDate = (dateString: string | Date, formatStr: string = "dd/MM/yyyy"): string => {
  try {
    let date: Date;
...
export function CalendarAdmin() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Gestión de Calendario Académico</CardTitle>
      </CardHeader>
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    ...
                    <div key={semester._id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {safeFormatDate(semester.startDate)} - {safeFormatDate(semester.endDate)}
                        </p>
                        {semester.isActive && (
          ...
          {/* Gestión de Semanas de Examen */}
          <TabsContent value="exam-weeks" className="space-y-6 mt-6">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <CalendarIcon className="h-5 w-5" />
                  ...
                  {examWeeks.map((examWeek) => (
                    <div key={examWeek._id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {safeFormatDate(examWeek.startDate)} - {safeFormatDate(examWeek.endDate)}
                        </p>
                      </div>
                      <Button
                        variant="destructive"
