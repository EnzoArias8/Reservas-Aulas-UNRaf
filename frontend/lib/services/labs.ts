...
export class LabService {
  static async getLabs(filters?: LabFilters): Promise<Lab[]> {
    ...
    const response = await axiosClient.get<ApiResponse<Lab[]>>(endpoint)
    return response.data?.data || response.data
  }

  static async getLabById(id: string): Promise<Lab> {
    const response = await axiosClient.get<ApiResponse<Lab>>(`/labs/${id}`)
    return response.data?.data || response.data
  }
  ...
  static async createLab(labData: Omit<Lab, "id" | "createdAt" | "updatedAt">): Promise<Lab> {
    const response = await axiosClient.post<ApiResponse<Lab>>("/labs", labData)
    return response.data?.data || response.data
  }

  static async updateLab(id: string, labData: Partial<Lab>): Promise<Lab> {
    const response = await axiosClient.put<ApiResponse<Lab>>(`/labs/${id}`, labData)
    return response.data?.data || response.data
  }

  static async deleteLab(id: string): Promise<void> {
    await axiosClient.delete(`/labs/${id}`)
  }
}
